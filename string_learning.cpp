...
int main()
{
    ...
    // s.substr(start,length) or (start)      slice it form start. if length omitted,grabs till end
    // of string.

    string name = "Donald Knuth";
    if (name.find("Kun") != string::npos)
    {  // npos 表示 not a position 不是一个有效的位置它是一个特殊的值，
